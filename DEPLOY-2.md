...
- App mein "⚙ API key settings" se apni **Google Gemini** API key browser mein hi save hoti hai (localStorage) — yeh repo ka hissa nahi banti.
- Jab grader app kholega, wo apni khud ki key daal ke test kar sakta hai.
- Key **bilkul free** milti hai (koi card nahi chahiye): [aistudio.google.com/apikey](https://aistudio.google.com/apikey) par jao → Google account se login → "Create API Key" dabao.
