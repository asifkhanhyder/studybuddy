...
**Model used:** `gemini-flash-latest` via the Google Gemini API (free tier, no card required), called directly from the browser.
...
- **AI model:** Google Gemini (`gemini-flash-latest`) via the Gemini API free tier, called client-side
...
3. Click **⚙ API key settings** and paste your own free Google Gemini API key (get one at [aistudio.google.com/apikey](https://aistudio.google.com/apikey), no card required) — it's stored only in your browser's localStorage and is never committed to this repo
...
No API keys are committed anywhere in this repo. Each user (including graders) supplies their own free Google Gemini API key through the in-app settings panel, which is stored client-side only.
